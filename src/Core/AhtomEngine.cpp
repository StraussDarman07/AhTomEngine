...
void Core::AhTomEngine::createImageViews()
{
	...
	for (uint32_t i = 0; i < swapChainImages.size(); i++) {
		swapChainImageViews[i] = ImageUtils::createImageView(device, swapChainImages[i], swapChainImageFormat);
	}
}

void Core::AhTomEngine::createDescriptorSetLayout()
{
...
void Core::AhTomEngine::createUniformBuffers() {
...
void Core::AhTomEngine::createIndexBuffer()
{
...
void Core::AhTomEngine::updateUniformBuffer(uint32_t currentImage)
{
...
void Core::AhTomEngine::createDescriptorSets()
{
...
void Core::AhTomEngine::createTextureImage()
{
	...
	ImageUtils::createImage(device, physicalDevice, texWidth, texHeight, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, textureImage, textureImageMemory);

	Buffer::transitionImageLayout(device, commandPool, graphicsQueue, textureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
	Buffer::copyBufferToImage(device, commandPool, graphicsQueue, stagingBuffer, textureImage, static_cast<uint32_t>(texWidth), static_cast<uint32_t>(texHeight));
	Buffer::transitionImageLayout(device, commandPool, graphicsQueue, textureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);

	vkDestroyBuffer(device, stagingBuffer, nullptr);
	vkFreeMemory(device, stagingBufferMemory, nullptr);
}

void Core::AhTomEngine::createTextureImageView()
{
	textureImageView = ImageUtils::createImageView(device, textureImage, VK_FORMAT_R8G8B8A8_SRGB);
}

void Core::AhTomEngine::createTextureSampler() {
